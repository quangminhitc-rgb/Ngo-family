...
import { Upload, Trash2, Edit2, Check, X, Image, Plus, FolderOpen, Calendar, FolderPlus, Layers, CheckSquare, Square } from 'lucide-react'
...
export default function AdminPhotosPage() {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const remaining = UPLOAD_LIMIT - previewFiles.length
    if (remaining <= 0) {
      setUploadError(`Giới hạn ${UPLOAD_LIMIT} ảnh mỗi lần upload. Hãy upload batch hiện tại trước.`)
      e.target.value = ''
      return
    }
    const allowed = files.slice(0, remaining)
    if (files.length > remaining) {
      setUploadError(`Chỉ thêm được ${allowed.length} ảnh (giới hạn ${UPLOAD_LIMIT} ảnh/lần).`)
    }
  ...
  const toggleSelectAll = () => {
    const deletableIds = filteredPhotos
      .filter(p => session?.user.role === 'admin' || p.userId === session?.user.id)
      .map(p => p.id)
  ...
  const handleCreateAlbum = async () => {
    ...
    try {
      ...
      if (res.ok) { setNewAlbumName(''); setNewAlbumDesc(''); fetchAlbums() }
  ...
  const handleDeleteAlbum = async (id: string, name: string) => {
    ...
    if (res.ok) { fetchAlbums(); if (selectedAlbum === id) setSelectedAlbum(null) }
  }

  const canDelete = (photo: Photo) =>
    session?.user.role === 'admin' || photo.userId === session?.user.id

  const deletableCount = filteredPhotos.filter(p => canDelete(p)).length
  ...
  const albumTabClass = (active: boolean) =>
    `w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg text-sm transition-all text-left ${
      active ? 'bg-[#c9a84c]/15 text-[#c9a84c] font-medium' : 'text-[#888] hover:text-white hover:bg-[#1a1a1a]'
    }`
  ...
  const mobileTabClass = (active: boolean) =>
    `flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition-all whitespace-nowrap ${
      active ? 'bg-[#c9a84c]/15 text-[#c9a84c] border-[#c9a84c]/30' : 'bg-[#111] text-[#888] border-[#2a2a2a] hover:text-white hover:border-[#333]'
    }`
      ...
      {previewFiles.length > 0 && (
        <div className="mb-6 p-5 rounded-2xl bg-[#111] border border-[#1a1a1a]">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-2">
              <h3 className="font-semibold text-white">Xem trước ({previewFiles.length}/{UPLOAD_LIMIT} ảnh)</h3>
              {previewFiles.length < UPLOAD_LIMIT && (
                <button
                  onClick={() => fileInputRef.current?.click()}
                  className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs text-[#666] hover:text-[#c9a84c] border border-[#2a2a2a] hover:border-[#c9a84c]/30 transition-all"
                >
                  <Plus size={11} /> Thêm
                </button>
              )}
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button onClick={() => setPreviewFiles([])} className="px-3 py-1.5 rounded-lg text-sm text-[#666] hover:text-[#e05252] transition-colors">
                Hủy
              </button>
              <button
              ...
              >
                {uploading ? <div className="w-4 h-4 border-2 border-[#0f0f0f]/50 border-t-[#0f0f0f] rounded-full animate-spin" /> : <Upload size={15} />}
              ...
              <div key={i} className="relative group rounded-xl overflow-hidden bg-[#1a1a1a] border border-[#222]">
                ...
                <div className="p-2 space-y-1.5">
                  <input
                    type="text"
                    value={pf.caption}
                    onChange={e => { const u = [...previewFiles]; u[i].caption = e.target.value; setPreviewFiles(u) }}
                  ...
                  <input
                    type="date"
                    value={pf.takenAt}
                    onChange={e => { const u = [...previewFiles]; u[i].takenAt = e.target.value; setPreviewFiles(u) }}
      ...
      <div className="flex gap-5">
        ...
        <div className="hidden lg:block w-48 flex-shrink-0">
          <div className="sticky top-4 rounded-2xl bg-[#111] border border-[#1a1a1a] overflow-hidden">
            ...
            <div className="p-2 space-y-0.5">
              <button onClick={() => setSelectedAlbum(null)} className={albumTabClass(selectedAlbum === null)}>
                <span className="flex items-center gap-2 min-w-0"><Layers size={14} className="flex-shrink-0" /><span className="truncate">Tất cả</span></span>
                <span className="text-xs text-[#555] flex-shrink-0">{photos.length}</span>
              </button>
              {albums.map(album => (
                <button key={album.id} onClick={() => setSelectedAlbum(album.id)} className={albumTabClass(selectedAlbum === album.id)}>
                  <span className="flex items-center gap-2 min-w-0"><FolderOpen size={14} className="flex-shrink-0" /><span className="truncate">{album.name}</span></span>
                  <span className="text-xs text-[#555] flex-shrink-0">{albumCounts[album.id] ?? 0}</span>
                </button>
              ))}
              {(albumCounts['none'] ?? 0) > 0 && (
                <button onClick={() => setSelectedAlbum('none')} className={albumTabClass(selectedAlbum === 'none')}>
                  <span className="flex items-center gap-2 min-w-0"><Image size={14} className="flex-shrink-0" /><span className="truncate">Chưa phân loại</span></span>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="lg:hidden flex gap-2 overflow-x-auto pb-2 mb-4 scrollbar-hide">
            <button onClick={() => setSelectedAlbum(null)} className={mobileTabClass(selectedAlbum === null)}>Tất cả <span className="text-[#555]">{photos.length}</span></button>
            {albums.map(album => (
            ...
            {(albumCounts['none'] ?? 0) > 0 && (
              <button onClick={() => setSelectedAlbum('none')} className={mobileTabClass(selectedAlbum === 'none')}>Chưa phân loại <span className="text-[#555]">{albumCounts['none']}</span></button>
            )}
          </div>

          {/* Toolbar: album label + select toggle */}
          <div className="flex items-center justify-between mb-3 min-h-[28px]">
            <div className="flex items-center gap-2">
              {selectedAlbum !== null && (
                <>
                  <p className="text-sm font-medium text-white">
                    {selectedAlbum === 'none' ? 'Chưa phân loại' : albums.find(a => a.id === selectedAlbum)?.name ?? ''}
                    <span className="ml-2 text-[#555] font-normal">{filteredPhotos.length} ảnh</span>
                  </p>
                  <button onClick={() => setSelectedAlbum(null)} className="text-xs text-[#555] hover:text-[#888]">Xem tất cả</button>
                </>
              )}
            </div>
            {filteredPhotos.length > 0 && deletableCount > 0 && (
              <button
                onClick={() => { selectMode ? exitSelectMode() : setSelectMode(true) }}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                  selectMode
                    ? 'bg-[#c9a84c]/10 text-[#c9a84c] border-[#c9a84c]/30'
                    : 'text-[#666] border-[#2a2a2a] hover:text-white hover:border-[#333]'
                }`}
              >
                <CheckSquare size={13} />
          ...
          {selectMode && (
            <div className="flex flex-wrap items-center justify-between gap-2 mb-4 px-4 py-3 rounded-xl bg-[#1a1a1a] border border-[#2a2a2a]">
              <div className="flex items-center gap-3">
                <button
                  onClick={toggleSelectAll}
                  className="flex items-center gap-1.5 text-sm text-[#a0a0a0] hover:text-white transition-colors"
                >
                  {selectedIds.size === deletableCount && deletableCount > 0
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-medium disabled:opacity-40 transition-all"
                style={{ background: 'rgba(224,82,82,0.15)', color: '#e05252', border: '1px solid rgba(224,82,82,0.3)' }}
              >
                {deleting
                  ? <div className="w-4 h-4 border-2 border-[#e05252]/40 border-t-[#e05252] rounded-full animate-spin" />
                  : <Trash2 size={14} />}
                Xóa {selectedIds.size > 0 ? selectedIds.size : ''} ảnh
              </button>
            </div>
          )}
          ...
          {loading ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-3">
              {Array.from({ length: 10 }).map((_, i) => <div key={i} className="skeleton rounded-xl aspect-square" />)}
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <p className="text-[#666]">{selectedAlbum !== null ? 'Album này chưa có ảnh' : 'Chưa có ảnh nào'}</p>
              <button
                onClick={() => fileInputRef.current?.click()}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-3">
              {filteredPhotos.map(photo => {
                ...
                return (
                  <div
                    key={photo.id}
                    onClick={() => selectMode && deletable && toggleSelect(photo.id)}
                    className={`group relative rounded-xl overflow-hidden bg-[#111] border transition-all ${
                      selectMode && deletable ? 'cursor-pointer' : ''
                    } ${
                      isSelected
                    ...
                    {selectMode && deletable && (
                      <div className="absolute top-1.5 left-1.5">
                        <div className={`w-5 h-5 rounded-md flex items-center justify-center border-2 transition-all ${
                          isSelected ? 'bg-[#c9a84c] border-[#c9a84c]' : 'bg-black/50 border-white/50'
                        }`}>
                          {isSelected && <Check size={11} className="text-[#0f0f0f]" strokeWidth={3} />}
                        </div>
                      </div>
                    )}

                    {/* Album badge */}
                    {!selectMode && selectedAlbum === null && photo.album && (
                    ...
                    {!selectMode && (
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-2">
                        ...
                        <div>
                          ...
                          <p className="text-white/50 text-xs mt-0.5">{formatDate(photo.takenAt ?? photo.uploadedAt)}</p>
                        </div>
                      </div>
                    )}

                    {/* Edit overlay */}
                    {!selectMode && editingId === photo.id && (
                      <div className="absolute inset-0 bg-[#111]/97 flex flex-col justify-center p-3 gap-2 overflow-y-auto">
                        ...
                        <div className="flex gap-1.5">
                          <button onClick={() => handleSaveEdit(photo.id)} className="flex-1 py-1.5 rounded-lg bg-[#c9a84c] text-[#0f0f0f] text-xs font-semibold flex items-center justify-center gap-1">
                            <Check size={12} /> Lưu
                          </button>
                          <button onClick={() => setEditingId(null)} className="flex-1 py-1.5 rounded-lg bg-[#1a1a1a] border border-[#2a2a2a] text-[#a0a0a0] text-xs flex items-center justify-center gap-1">
                            <X size={12} /> Hủy
                          </button>
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in"
          ...
          <div className="bg-[#111] border border-[#1a1a1a] rounded-2xl w-full max-w-md shadow-modal animate-slide-up">
            <div className="flex items-center justify-between p-5 border-b border-[#1a1a1a]">
              ...
              <button onClick={() => setShowAlbumModal(false)} className="w-7 h-7 rounded-lg bg-[#1a1a1a] flex items-center justify-center text-[#666] hover:text-white transition-colors">
                <X size={15} />
              </button>
            </div>
            <div className="p-5 space-y-4">
              <div className="space-y-2">
                <input type="text" value={newAlbumName} onChange={e => setNewAlbumName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleCreateAlbum()} placeholder="Tên album mới *"
                ...
                <input type="text" value={newAlbumDesc} onChange={e => setNewAlbumDesc(e.target.value)} placeholder="Mô tả (tuỳ chọn)"
                  className="w-full px-3 py-2.5 bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl text-white placeholder-[#444] text-sm focus:outline-none focus:border-[#c9a84c]/40" />
                <button onClick={handleCreateAlbum} disabled={savingAlbum || !newAlbumName.trim()}
                  ...
                  {savingAlbum ? <div className="w-4 h-4 border-2 border-[#0f0f0f]/50 border-t-[#0f0f0f] rounded-full animate-spin" /> : <Plus size={15} />}
                  Tạo album
                </button>
              </div>
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {albums.length === 0 ? (
                  ...
                  <div key={album.id} className="flex items-center gap-3 p-3 rounded-xl bg-[#1a1a1a] border border-[#222] group">
                    <FolderOpen size={16} className="text-[#c9a84c] flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-white font-medium truncate">{album.name}</p>
                      <p className="text-[10px] text-[#555]">{albumCounts[album.id] ?? 0} ảnh{album.isDefault ? ' · Album mặc định' : ''}</p>
                    </div>
                    {!album.isDefault && (
                      <button onClick={() => handleDeleteAlbum(album.id, album.name)}
